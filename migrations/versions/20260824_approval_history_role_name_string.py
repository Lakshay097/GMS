"""Change discrepancy_approval_history.assigned_role_id from UUID to role name string.

Previously stored a UUID referencing a (non-existent) roles table.
Now stores the role name string directly (e.g., 'admin', 'checker').
"""

from alembic import op
import sqlalchemy as sa

revision = "20260824_approval_history_role"
down_revision = "20260824_escalation_role_name"
branch_labels = None
depends_on = None


def upgrade() -> None:
    op.alter_column(
        "discrepancy_approval_history",
        "assigned_role_id",
        type_=sa.String(50),
        existing_nullable=True,
    )


def downgrade() -> None:
    op.alter_column(
        "discrepancy_approval_history",
        "assigned_role_id",
        type_=sa.dialects.postgresql.UUID(as_uuid=True),
        existing_nullable=True,
    )
