...
class TaskService:
    ...
    async def create_task(
        ...
        *,
        ...
        task = Task(
            ...
            completion_rule=completion_rule,      # immutable hereafter
            eta=eta,
        ...
        self.db.add(task)
        await self.db.flush()  # get task.id before owners
    ...
    async def complete_task(
        ...
        *,
        ...
        # Record per-owner completion (idempotent — ignore duplicate)
        ...
        if task.completion_rule == TaskCompletionRule.ANY_OWNER:
            ...
            if owners <= completed_ids:          # all owners done
                task.status = TaskStatus.COMPLETED
                task.completed_at = now
    ...
    async def update_completion_rule(self, task_id: UUID, new_rule: TaskCompletionRule) -> None:
        ...
        Provided as an explicit rejection endpoint so callers get a structured error.
        """
        task = await self._get_task_or_404(task_id)
        raise BusinessRuleError(
    ...
    async def request_eta_extension(
        ...
        *,
        ...
        """
        Request an ETA extension on a task.

        R-33/BR-10/C8/R-42:
          - Extensions 1–3 are granted and the ETA is updated.
          - A 4th request (eta_extension_count already == MAX_ETA_EXTENSIONS == 3)
            is NEVER granted; it is auto-converted to a TaskEscalation with
            trigger="fourth_extension_request" and the ETA remains unchanged.
        """
        task = await self._get_task_or_404(task_id)
        ...
        # 4th attempt — auto-escalate instead of granting (R-33/BR-10)
        ...
        # Validate new ETA is in the future
        if new_eta <= now:
            raise ValidationError(
        ...
        # Validate new ETA is later than current ETA
        if new_eta <= task.eta:
            raise ValidationError(
        ...
        # Notify all owners of the updated ETA
        owners = await self._get_owner_ids(task_id)
    ...
    async def _auto_escalate_fourth_extension(
        ...
        *,
        ...
        The task ETA is NOT updated; the request is logged with outcome="auto_escalated".
        The task status transitions to ESCALATED.
        All owners receive a mandatory ESCALATION (cat 1) notification.
        """
        # Record the rejected extension attempt
        self.db.add(
            TaskEtaExtension(
                id=uuid.uuid4(),
                task_id=task.id,
        ...
        # Mandatory ESCALATION notification (cat 1 — cannot be muted per R-39/C9)
